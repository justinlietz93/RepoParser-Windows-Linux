...
class TreeView:
    ...
    def render(self, tree, ignored_dirs=None, ignored_files=None):
        """Render the VS Code-style tree view component."""
        ...
        # Render the component with a more appropriate height for sidebar
        component_key = f"tree_view_{hash(json.dumps(tree))}"
        components.html(html, height=400, scrolling=True, key=component_key)
        ...
        return st.session_state[component_key]['ignored_dirs'], st.session_state[component_key]['ignored_files'] 